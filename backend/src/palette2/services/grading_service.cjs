            ...
            return await this.db.runTransaction(async (db) => {
                ...
                if (!assessment) {
                    ...
                    await db.run(`
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    `, [
                        assessmentId, submissionId, gradingData.rubric_template_id,
                        userId, 0, now, now
                    ]);
                } else {
                    assessmentId = assessment.id;

                    // Clear existing criterion assessments
                    await db.run(`
                        DELETE FROM criterion_assessments WHERE assessment_id = ?
                    `, [assessmentId]);
                }

                // Process criterion grades
                let totalScore = 0;
                for (const criterionGrade of gradingData.criterionGrades || []) {
                    const criterionAssessmentId = uuidv4();
